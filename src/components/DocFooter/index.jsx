...
function getNavigationPages(currentPath, sidebarData) {
  ...
  const currentIndex = allItems.findIndex((item) => {
    if (item.type === "link") {
      return currentPath === item.href || (item.href === "/" && currentPath === "/");
    } else if (item.type === "doc") {
  ...
  if (currentIndex > 0) {
    ...
    if (prevItem.type === "link") {
      previousPage = {href: prevItem.href, title: capitalizeFirst(prevItem.label)};
    } else if (prevItem.type === "doc") {
      previousPage = {href: `/docs/${prevItem.id}`, title: capitalizeFirst(prevItem.label)};
    }
  }
  ...
  if (currentIndex < allItems.length - 1) {
    ...
    if (nextItem.type === "link") {
      nextPage = {href: nextItem.href, title: capitalizeFirst(nextItem.label)};
    } else if (nextItem.type === "doc") {
      nextPage = {href: `/docs/${nextItem.id}`, title: capitalizeFirst(nextItem.label)};
    }
  }
...
export default function DocFooter() {
  ...
  const {previousPage, nextPage} = getNavigationPages(location.pathname, sidebarData);
  ...
  return (
    <div className={styles.footer}>
      {previousPage ? (
        <Link to={previousPage.href} className={styles.navLink} title={previousPage.title}>
      ...
      {nextPage ? (
        <Link to={nextPage.href} className={`${styles.navLink} ${styles.navLinkNext}`} title={nextPage.title}>
