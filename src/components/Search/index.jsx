...
export default function SearchBar({searchId, className, placeholder}) {
  ...
  const getContentPreview = (content, query) => {
    ...
    const filteredLines = lines.filter((line) => {
      ...
      const exportPattern = /^export\s+(default\s+)?(function|const|class|let|var)/;
  ...
  const getSectionContent = (content, sectionTitle, query, hit) => {
    ...
    const afterSection = text.substring(sectionIndex + sectionTitle.length).trim();
    
    let endIndex = afterSection.length;
    
    if (hit.headings && Array.isArray(hit.headings)) {
      const currentIndex = hit.headings.findIndex((h) => 
        h && h.toLowerCase() === sectionLower
      );
      ...
      if (currentIndex !== -1 && currentIndex < hit.headings.length - 1) {
        ...
        if (nextHeading) {
          ...
          const nextHeadingIndex = afterSection.toLowerCase().indexOf(nextHeadingLower);
    ...
    const sectionContent = afterSection.substring(0, Math.min(endIndex, 200)).trim();
          ...
          {isLoading ? (
            ...
            <div className={styles.results}>
              {results.map((hit) => {
                ...
                const displayTitle = matchingSection || hit.title || hit.hierarchy?.lvl0 || "";
                const parentTitle = matchingSection ? (hit.title || hit.hierarchy?.lvl0 || "") : null;
                const sectionContent = matchingSection ? getSectionContent(hit.content, matchingSection, query, hit) : null;
                const contentToShow = sectionContent || (hit.content ? getContentPreview(hit.content, query) : "");
